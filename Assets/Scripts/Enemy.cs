...
public class Enemy : MonoBehaviour
{
    ...
    public enum EnemyState { Home, Chase, Scatter, Scared, Res, Dead }
    public EnemyState state = EnemyState.Home;
    public bool isActive => state == EnemyState.Chase || state == EnemyState.Scatter;
    ...
    public float baseSpeedModifier = .9f;
    ...
    public void Trigger(string triggerName = null, EnemyState? state = null) {
        ...
        if (this.state == EnemyState.Home || isActive) {
            movement.moveSpeed = baseSpeedModifier*levelManager.baseMoveSpeed;
        } else {
    ...
    void CheckState() {
        ...
        else if (levelManager.levelState == LevelManager.GameState.Normal && !(state == EnemyState.Chase || state == EnemyState.Scatter)) {
            if (levelManager.isInnov) {
                Trigger("normal", levelManager.lastAttackState);
            } else {
                Trigger("normal", EnemyState.Chase);
            }
